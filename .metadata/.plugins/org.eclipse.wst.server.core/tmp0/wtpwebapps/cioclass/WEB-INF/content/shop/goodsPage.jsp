...
/*get page data*/
function tos(num){
	turn_to_page(config.serverIp+"shop/getGoodsPage",num);
}
...
function del(id){
	if(window.confirm('你确定要删除id为'+id+'的记录？')){
		$.getJSON(config.serverIp+"shop/delGoodsById",{"id":id},function(data){
...
/*getOne function*/
function get(id){
	$.getJSON(config.serverIp+"shop/getOneGoods",{"id":id},function(data){
		var msg=$.parseJSON(data).msg;
		if(msg!=null && msg=='OK'){
			alert(data);
		}else{
